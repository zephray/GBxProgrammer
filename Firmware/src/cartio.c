...
__attribute__((always_inline)) void gb_res_cs2_high(void) {
    gpio_set(CART_GB_RES_CS2_PORT, CART_GB_RES_CS2_PIN);
}

__attribute__((always_inline)) void gb_res_cs2_low(void) {
    gpio_clear(CART_GB_RES_CS2_PORT, CART_GB_RES_CS2_PIN);
}

__attribute__((always_inline)) void gb_ain_irq_high(void) {
    gpio_set(CART_GB_AIN_IRQ_PORT, CART_GB_AIN_IRQ_PIN);
}

__attribute__((always_inline)) void gb_ain_irq_low(void) {
    gpio_clear(CART_GB_AIN_IRQ_PORT, CART_GB_AIN_IRQ_PIN);
}

__attribute__((always_inline)) void gb_wr_high(void) {
    gpio_set(CART_GB_WR_PORT, CART_GB_WR_PIN);
}

__attribute__((always_inline)) void gb_wr_low(void) {
    gpio_clear(CART_GB_WR_PORT, CART_GB_WR_PIN);
}

__attribute__((always_inline)) void gb_rd_high(void) {
    gpio_set(CART_GB_RD_PORT, CART_GB_RD_PIN);
}

__attribute__((always_inline)) void gb_rd_low(void) {
    gpio_clear(CART_GB_RD_PORT, CART_GB_RD_PIN);
}

__attribute__((always_inline)) void gb_cs_high(void) {
    gpio_set(CART_GB_CS_PORT, CART_GB_CS_PIN);
}

__attribute__((always_inline)) void gb_cs_low(void) {
    gpio_clear(CART_GB_CS_PORT, CART_GB_CS_PIN);
}

#ifdef HW_R2
//GBA Support start from Hardware Revision 2.0
__attribute__((always_inline)) void gba_d0_high(void) {
    gpio_set(CART_GBA_D0_PORT, CART_GBA_D0_PIN);
}

__attribute__((always_inline)) void gba_d0_low(void) {
    gpio_clear(CART_GBA_D0_PORT, CART_GBA_D0_PIN);
}

__attribute__((always_inline)) void gb_a_set_input(void) {
    gpio_set_mode(GPIOB, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT, GPIO_ALL);
    gpio_clear(CART_GB_A_DIR_PORT, CART_GB_A_DIR_PIN);
}

__attribute__((always_inline)) void gb_a_set_output(void) {
...
__attribute__((always_inline)) uint16_t gb_a_read(void) {
    return gpio_port_read(GPIOB);
}
#endif

__attribute__((always_inline)) void gb_a_write(uint16_t addr) {
    gpio_port_write(GPIOB, addr);
}

__attribute__((always_inline)) void gb_d_set_input(void) {
    gpio_set_mode(GPIOA, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT, 0x00FF);// GPIO0-7
    gpio_clear(CART_GB_D_DIR_PORT, CART_GB_D_DIR_PIN);
}

__attribute__((always_inline)) void gb_d_set_output(void) {
    gpio_set(CART_GB_D_DIR_PORT, CART_GB_D_DIR_PIN);
    gpio_set_mode(GPIOA, GPIO_MODE_OUTPUT_10_MHZ, GPIO_CNF_OUTPUT_PUSHPULL, 0x00FF);
}

__attribute__((always_inline)) uint8_t gb_d_read(void) {
    return (gpio_port_read(GPIOA) & 0xFF);
}

__attribute__((always_inline)) void gb_d_write(uint8_t data) {
    GPIOA_ODR = ((GPIOA_ODR & 0xFF00) | (data & 0xFF));
}

#if HW_R2
__attribute__((always_inline)) void gba_a_write(uint32_t addr) {
    gb_d_write(addr >> 16);
...
void cart_set_gb_mode() {
    gb_d_set_input();
    gb_a_set_output();
}

uin8_t cart_gb_read(uint16_t addr, bool ram_access) {
    gb_a_write(addr);
